              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="group w-full gold-gradient text-primary-foreground font-display font-bold text-sm tracking-[0.12em] uppercase py-4 rounded-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-3 shadow-lg shadow-primary/20 disabled:opacity-60"
                  >
                    {loading ? "Sending…" : <>Submit Enquiry<ArrowRight size={15} className="transition-transform duration-300 group-hover:translate-x-1" /></>}
                  </button>

                  {error && (
                    <p className="text-center text-[12px] text-red-500 font-body -mt-2">{error}</p>
                  )}
